...
test("single-server MCP exposes full inventory and guidance without SSH", async () => {
  ...
  await withClient(server, async (client) => {
    ...
    assert.deepEqual(result.structuredContent.data, saved);
...
test("fleet MCP lists per-host inventory, null for missing info, no secrets, and startup guidance", async () => {
  ...
  try {
    ...
    await withClient(runtime.server, async (client) => {
      ...
      assert.ok(descriptionCharacters < 12_000);
      assert.ok(!JSON.stringify(tools).includes(info.usageGuidance));
      const result = await client.callTool({name: "list_servers", arguments: {}});
      assert.ok(!result.isError);
      const rows = JSON.parse(result.content[0].text);
      assert.deepEqual(result.structuredContent.data, rows);
      ...
      for (const name of ["one", "two"]) {
        ...
        assert.deepEqual(detail.structuredContent.data.server_info, name === "one" ? info : null);
      ...
      const missingText = JSON.parse(missing.content[0].text);
      assert.equal(missing.structuredContent.error, missingText.error);
    });
